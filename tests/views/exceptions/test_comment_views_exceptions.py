class TestCommentViewsExceptions:
    def test_put_comment_exceptions(self, client, comment_1, comment_2, headers_user, comment_service):
        ...
        response_1 = client.put(f'/comments/{comment_1.id}')
        ...
        response_2 = client.put(f'/comments/{comment_2.id}', json=data_2, headers=headers_user)
        assert response_2.status_code == 403, f'Возвращается код {response_2.status_code} вместо 403'

        # Обращение без данных
        response_3 = client.put(f'/comments/{comment_1.id}', headers=headers_user)
        ...
        response_4 = client.put(f'/comments/{comment_1.id}', json=data_4, headers=headers_user)
        ...
        response_5 = client.put(f'/comments/{comment_1.id}', json=data_5, headers=headers_user)
        assert response_5.status_code == 400, f'Возвращается код {response_5.status_code} вместо 400'

        data_6 = {
            "text": ""
        }
        response_6 = client.put(f'/comments/{comment_1.id}', json=data_6, headers=headers_user)
        ...
        response_7 = client.put(f'/comments/99999', json=data_7, headers=headers_user)
    ...
    def test_delete_exceptions(self, client, comment_1, comment_2, headers_admin, user_2, headers_user):
        ...
        # Обращение не автора комментария
        response_2 = client.delete(f'/comments/{comment_2.id}', headers=headers_user)
        assert response_2.status_code == 403, f'Возвращается код {response_2.status_code} вместо 403'

        # Несуществующий комментарий
        response_2 = client.delete(f'/comments/999999', headers=headers_user)
