class TestNewsViewsExceptions:
    ...
    def test_post_news_exceptions(self, client, headers_user):
        ...
        response_3 = client.post(self.url, json=data_3, headers=headers_user)
        ...
        response_4 = client.post(self.url, json=data_4, headers=headers_user)
        assert response_4.status_code == 400, f'Возвращается код {response_4.status_code} вместо 400'

        data_5 = {
            "text": ""
        }
        response_5 = client.post(self.url, json=data_5, headers=headers_user)
    ...
    def test_get_one_exceptions(self, client, news_1, headers_user):
        # Неавторизованный пользователь
        response_1 = client.get(f'{self.url}{news_1.id}/')
        assert response_1.status_code == 401, f'Возвращается код {response_1.status_code} вместо 401'

        # Несуществующая новость
        response_2 = client.get(f'{self.url}99999/', headers=headers_user)
    ...
    def test_post_comment_exceptions(self, client, news_1, headers_user):
        # Неавторизованный пользователь
        response_1 = client.post(f'{self.url}{news_1.id}/')
        assert response_1.status_code == 401, f'Возвращается код {response_1.status_code} вместо 401'
        ...
        response_2 = client.post(f'{self.url}99999/', json=data_2, headers=headers_user)
        assert response_2.status_code == 404, f'Возвращается код {response_2.status_code} вместо 404'

        # Обращение без данных
        response_3 = client.post(f'{self.url}{news_1.id}/', headers=headers_user)
        assert response_3.status_code == 400, f'Возвращается код {response_3.status_code} вместо 400'
        ...
        response_4 = client.post(f'{self.url}{news_1.id}/', json=data_4, headers=headers_user)
        assert response_4.status_code == 400, f'Возвращается код {response_4.status_code} вместо 400'
        ...
        response_5 = client.post(f'{self.url}{news_1.id}/', json=data_5, headers=headers_user)
        assert response_5.status_code == 400, f'Возвращается код {response_5.status_code} вместо 400'

        data_6 = {
            "text": ""
        }
        response_6 = client.post(f'{self.url}{news_1.id}/', json=data_6, headers=headers_user)
    ...
    def test_put_news_exceptions(self, client, news_1, news_2, user_1, user_2, headers_user):
        # Неавторизованный пользователь
        response_1 = client.put(f'{self.url}{news_1.id}/')
        assert response_1.status_code == 401, f'Возвращается код {response_1.status_code} вместо 401'
        ...
        response_2 = client.put(f'{self.url}99999/', json=data_2, headers=headers_user)
        assert response_2.status_code == 404, f'Возвращается код {response_2.status_code} вместо 404'

        # Обращение без данных
        response_3 = client.put(f'{self.url}{news_1.id}/', headers=headers_user)
        assert response_3.status_code == 400, f'Возвращается код {response_3.status_code} вместо 400'
        ...
        response_4 = client.put(f'{self.url}{news_1.id}/', json=data_4, headers=headers_user)
        assert response_4.status_code == 400, f'Возвращается код {response_4.status_code} вместо 400'
        ...
        response_5 = client.put(f'{self.url}{news_1.id}/', json=data_5, headers=headers_user)
        assert response_5.status_code == 400, f'Возвращается код {response_5.status_code} вместо 400'

        data_6 = {
            "text": ""
        }
        response_6 = client.put(f'{self.url}{news_1.id}/', json=data_6, headers=headers_user)
        assert response_6.status_code == 400, f'Возвращается код {response_6.status_code} вместо 400'
        ...
        response_7 = client.put(f'{self.url}{news_2.id}/', json=data_7, headers=headers_user)
    ...
    def test_delete_news_exceptions(self, client, news_1, news_2, headers_user):
        # Неавторизованный пользователь
        response_1 = client.delete(f'{self.url}{news_1.id}/')
        assert response_1.status_code == 401, f'Возвращается код {response_1.status_code} вместо 401'

        # Несуществующая новость
        response_2 = client.delete(f'{self.url}99999/', headers=headers_user)
        assert response_2.status_code == 404, f'Возвращается код {response_2.status_code} вместо 404'

        # Обращение не автора новости
        response_3 = client.delete(f'{self.url}{news_2.id}/', headers=headers_user)
